...
main:
  ...
  {
    let top_node =
      if  "" = !Flags.user_specified_main_node_name
      then
	match !main_node with
	    Some n -> n
	  | None -> !current_node_id
      else
	let main_node_name = !Flags.user_specified_main_node_name in
...
| SUBRANGE LSQBRACKET real COMMA real RSQBRACKET OF REAL
  {
    Globals.is_inter := true;
    let i_1,n_1,d_1 = $3 in
    let i_2,n_2, d_2 = $5 in
    L_REAL_RANGE(i_1,n_1,d_1,i_2,n_2,d_2)
    ;
(*   L_INT*)
  }
| sym_name
...
(* Teme *)
  ...
  let stored_property =
      try Hashtbl.find nodes_property_list !current_node_id;
      with Not_found -> (S_TRUE,L_BOOL)  in
    Globals.prop_typed_stream := $3;
    let new_property =  (S_AND ($3,stored_property),L_BOOL)  in
      Hashtbl.replace nodes_property_list !current_node_id new_property;
     multi_property_list := ($3 :: !multi_property_list);
    ()
...
| METAPROPERTY SEMICOLON
  {
    ...
    let index =
      ...
      with Not_found -> -1
    in
    if index >= 0 then Flags.set_use_x();
    cvcl_property := Lus_convert.f_and !cvcl_property (F_STR $1);
    ()
  }
| MAIN_NODE COLON TRUE SEMICOLON
  {
