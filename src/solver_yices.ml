...
let subrange_real i n d =
  if i != 0 && n != 0 then
    let ftmp = (float_of_int i)+.(float_of_int n)/.(float_of_int d) in
    "(+ " ^ string_of_int(i) ^  " (/ " ^ (string_of_int n)^ " " ^(string_of_int d) ^ " ))"
  else if i = 0 then
...
class solver_yices = object (self)
  ...
  | L_REAL_RANGE(i_1, n_1, d_1, i_2, n_2, d_2) ->
     begin
       let range_min = subrange_real i_1 n_1 d_1 in
       let range_max = subrange_real i_2 n_2 d_2 in
       "(subrange "^ range_min ^ " " ^ range_max ^")"
     end
  | L_TUPLE(y) ->
