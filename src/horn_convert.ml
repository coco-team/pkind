...
(* keep here *)
let simplify_var k pos alt mode pd solver =
   match k with
     NUM id ->
       begin
         try
           let (n,v,t,c) = Tables.safe_find_varinfo id "simplify_var" in
             begin
               match mode,c with
    ...
    | _ -> raise (ConversionError "simplify_var")
...
let rec expr_buffer mode e =
  ...
  begin
    match e with
                  ...
                  | DEPTH d ->
                     let d' =
                       if !do_negative_index then
                         ...
                         d
                     in
                     Buffer.add_buffer buf (expr_buffer mode (PLUS(STEP_BASE,NUM(d'))))
              end
      | INT_CONST(e1) ->
                         Buffer.add_buffer buf (expr_buffer mode e1)
      ...
      | PLUS(e1,NUM(n)) ->
          let b1 = expr_buffer mode e1 in
          if n < 0 then
            begin
              let b2 = expr_buffer mode (NUM (-n)) in
      ...
      | UMINUS(e1) ->
          if solver#get#supports_unary_minus then
            ...
            begin
              let b1 = expr_buffer mode (MINUS(NUM 0,e1)) in
      ...
      | ITE(e1,e2,e3) ->
          begin
            let test_buf = boolean_relation_buffer mode e1 solver#get#one_string in
            let b1 = expr_buffer mode e2 in
            let b2 = expr_buffer mode e3 in
            solver#get#buffer_of_ite buf test_buf b1 b2
          end
      | STREAM_ITE(e1,e2,e3) ->
          begin
             if !Globals.whichState then (
                Buffer.add_buffer buf (expr_buffer mode e2)
             ) else (
              Buffer.add_buffer buf (expr_buffer mode e3)
            (*  match mode,e1 with
      ...
      | B_IMPL(e1,e2) ->
          if solver#get#term_impl_available then
            ...
            Buffer.add_buffer buf (expr_buffer mode (B_OR((B_NOT e1),e2)))
      | B_NOT(e1) ->
          let b1 = expr_buffer mode e1 in
                  ...
                  begin
                    match mode with
                        FRESH_INPUTS x ->
                             ...
                             in
                             let p =
                               Buffer.contents (expr_buffer mode
                                 (MINUS(POSITION_VAR solver#get#position_var1,NUM(pd))))
                             in
                             let vs,_ =
                               (simplify_var k p "??" (FRESH_INPUTS pd) pd solver)
                             in
                             s.s <- vs;
...
(* used during k-induction *)
      | PRED(s1,li) ->
                      if List.length li > 0 then
                        begin
                          let li2 =
                            List.map (fun x -> expr_buffer mode x) li
                          in
	  ...
	  raise (ConversionError "expr_buffer")
  end;
  buf
and boolean_relation_buffer mode e rh_string =
  let buf = Buffer.create 1600 in
  begin
    match e with
        REL(EQ,e4,e5) ->
          let b1 = expr_buffer mode e4 in
          let b2 = expr_buffer mode e5 in
          solver#get#buffer_of_binary buf solver#get#eq_string b1 b2
      | REL(LT,e4,e5) ->
          let b1 = expr_buffer mode e4 in
          let b2 = expr_buffer mode e5 in
...
(* keep here *)
let add_def solver lhs rhs s1 s2 =
...
(*  let rhs_ids = List.map (fun (_,i)->i) rhs in*)
  ...
  match lhs with
    ...
    | _ -> raise (ConversionError "add_def")
...
(* keep here *)
let rec formula_string_buffer mode coi_depth f =
  ...
  begin
    match f with
      ...
      | F_EQ(c1,c2,t) -> add_def_term_hash c1 c2;
	                       clear_current_vars ();
                         let s1 = (expr_buffer mode c1) in
                         let lhs = get_current_vars () in

                         clear_current_vars ();
                         let s2 = (expr_buffer mode c2) in
                         ...
                         if mode != STATE_VAR_GEN then
                           add_def solver lhs rhs s1 s2

      | F_NOT(f1) -> let s1 = formula_string_buffer mode coi_depth f1 in
                     solver#get#buffer_of_unary buf solver#get#f_not_string s1
      | F_OR(f1,f2) -> let s1 = formula_string_buffer mode coi_depth f1 in
                       let s2 = formula_string_buffer mode coi_depth f2 in
                       solver#get#buffer_of_binary buf solver#get#f_or_string s1 s2
      | F_IMPL(f1,f2) -> let s1 = formula_string_buffer mode coi_depth f1 in
                       let s2 = formula_string_buffer mode coi_depth f2 in
...
slightly faster than a tail-recursive version *)
          ...
          let rec getlist fas =
                match fas with
                  ...
                  | _ -> raise (ConversionError "formula_string_buffer")
          in
          ...
          in
          let fl =
            List.map (fun x -> formula_string_buffer mode coi_depth x)
              (getlist f)
          in
    ...
    match f with
      ...
      | F_PRED (s1,li) ->
                        if List.length li > 0 then
                          begin
                            let li2 =
                              List.map (fun x -> expr_buffer GENERAL x) li
                            in
                            solver#get#buffer_of_pred buf s1 li2
                          end
                        else
                          Buffer.add_string buf s1
      | F_STR(s1) -> Buffer.add_string buf s1
      | F_EQ(c1,c2,t) -> let s1 = (expr_buffer GENERAL c1) in
                         let s2 = (expr_buffer GENERAL c2) in
                         let eq_str =
                           if t = L_BOOL then
                             solver#get#f_iff_string
                           else
...
let allVar () =
    ...
    let varDeclInit = ref "" in (* variable decls for init state*)
    let varDeclStep = ref "" in (* variable decls  for the transition relation *)
    let predTypes = ref "" in (* types of variables in the predicate definition *)
    let varInit = ref "" in (* variables for init state *)
    let varStep = ref "" in (* variables transition relation *)
    let allV = Hashtbl.fold (fun i (n,v,t,c) acc ->
    try
      let uvd = Tables.find_used_var i in
        begin
          let currentV = "("^v^"_pre "^(type_string t)^")" in
          let v' = "("^v^(type_string t)^")" in
          varInit := v^"_pre "^ !varInit;
          varStep := v^ !varStep;
          varDeclStep := v'^ !varDeclStep;
          varDeclInit := currentV^ !varDeclInit;
   (*        let _ = match uvd with
                      None -> ()

                      | Some _ ->
                          let v' = "("^v^"_nxt "^(type_string t)^")" in
                            varStep := v^"_nxt "^ !varStep;
                            varDeclStep := v'^ !varDeclStep;
                 in *)
          predTypes := (type_string t)^ " " ^ !predTypes;
          ...
          acc^solver#get#cc^first^currentV^"\t|->"^s3^(resolve_in_out c)^s4
          ^"/"^(string_of_int i)^"\n"
        end
    with Not_found -> acc
  ) (Tables.get_varinfo()) "" in
    let predDecl = "(declare-fun PRED (" ^ !predTypes ^ ") Bool)" in
    let predInit = "(PRED "^ !varInit ^")" in
    let predStep = "(PRED "^ !varStep ^")" in
    varDeclStep := !varDeclInit ^ " \n\t" ^ !varDeclStep;
    allV, predDecl, varDeclInit, varDeclStep, predInit, predStep
...
let allVarRule () =
    ...
    let varInit = ref "" in (* variables for init state *)
    let varStep = ref "" in (* variables transition relation *)
    let varDecl = ref [] in (* all variable declartion *)
    let allV = Hashtbl.fold (fun i (n,v,t,c) acc ->
    try
      let uvd = Tables.find_used_var i in
        begin
          let currentV = "(declare-var "^v^" "^(type_string t)^")" in
          let v' = "(declare-var "^v^"_nxt "^(type_string t)^")" in
          varDecl := currentV::v'::!varDecl;
          varInit := v^" "^ !varInit;
          varStep := v^"_nxt "^ !varStep;
          varDeclStep := v'^ !varDeclStep;
          varDeclInit := currentV^ !varDeclInit;
    ...
    let initDecl = "(declare-rel Init ())" in
    let errorDecl = "(declare-rel Error ())" in
    let predDecl = "(declare-rel PRED (" ^ !predTypes ^ "))" in
    let allVarsDecl = !varDecl in
    let predInit = "(PRED "^ !varInit ^")" in
    let predStep = "(PRED "^ !varStep ^")" in
    allV, predDecl, initDecl, errorDecl, allVarsDecl, predInit, predStep

(* keep here *)
let property_string_buf position p mode =
  let term,_ = Lus_convert.convert_term p position 0 0 None None in
  expr_buffer mode term

(* position_string, position, lustre_expr, cvc_expr *)
(* keep here*)

let property_header position_string position p =
  Buffer.contents (property_string_buf position p GENERAL)


let property_header_new position_string position p =
  let fs = Buffer.contents (property_string_buf position p GENERAL) in
  (* we need the guard implication for the step case with ite elim *)
  solver#get#property_header_new position_string fs


   let rec multi_property_header position_string position multi_p =
  match multi_p with
     [] -> ""
    |[x] -> Buffer.contents (property_string_buf position x GENERAL)
    | y::ys ->  "\n(define MULTI_PROPERTY ::(-> _nat bool) (lambda ("^position_string^"::_nat)"^"(and "^(String.concat " "
        (List.map
         (fun l->
           (Buffer.contents (property_string_buf position l GENERAL)))
         (y::ys)))^")))"

(* by Teme*)
(* To print a multiple properties as one string*)
let rec multi_property_header_list position multi_p =
  match multi_p with
    | [] -> []
    |[x] -> [Buffer.contents (property_string_buf position x GENERAL)]
    | y::ys ->  Buffer.contents (property_string_buf position y GENERAL):: multi_property_header_list position ys



let assumption_string position_string position =
  match Lus_assertions.get_assertion_term() with
      None -> ""
    | Some x -> let formula = expr_buffer GENERAL x in
                let buf = Buffer.create 6400 in
                solver#get#define_fun_buffer buf solver#get#assertions (M_FUNC[M_NAT;L_BOOL])
                  [(position_string,M_NAT)] formula;
                Buffer.contents buf

let assumption_string_invariant position_string position file =
  match Lus_assertions.get_assertion_term() with
      None -> ""
    | Some x -> let formula = expr_buffer GENERAL x in
                let buf = Buffer.create 6400 in
...
let state_vars_string () =
(* check against empty *)
  let buf = Buffer.create 6400 in
  let formula = formula_string_buffer STATE_VAR_GEN 0 (state_vars_tuples()) in
...
let state_vars_string_refined index =
  incr state_vars_version;
  let state_vars_string = get_state_vars_r_version () in
  let buf = Buffer.create 6400 in
  let formula =
    formula_string_buffer STATE_VAR_GEN 0 (state_vars_tuples_refined index)
  in
  match !Flags.set_my_solver with
  CVC4 ->
    begin
