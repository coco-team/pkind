...
let printTestInputOracle foundvars minstep maxstep =
  let minstep',maxstep' = if !do_negative_index then -maxstep,-minstep
  ...
  let buf = Buffer.create 100 in
   ...
   let _ = Buffer.add_string buf "    <testTrace>\n" in
    Hashtbl.iter (fun id _ ->
    let (_,key',_,c) = Tables.safe_find_varinfo id "printTestInput" in
      let buf2 = Buffer.create 20 in
      for i=minstep' to maxstep' do
        try
          ...
          if c=INPUT then(
            let keyIn = Tables.resolve_var_name key' in
            let tvalIn =
              try
                Hashtbl.find foundvars (id,i)
            with Not_found ->
                Hashtbl.find foundvars (id,-(i+1)) in
            let keyvalIn =
              if Str.string_match reg3 tvalIn 0 then "1"
              else if Str.string_match reg4 tvalIn 0 then "0"
              else if Str.string_match reg5 tvalIn 0 then "0"
              else if Str.string_match reg6 tvalIn 0 then "1"
              else  tvalIn in
            Buffer.add_string buf2 ("        <Value name=\""^keyIn^"\" type=\"input\" step=\""^(string_of_int (i+maxstep))^"\">"^keyvalIn^"</Value>\n");
          )else if (c=OUTPUT) then (
            let keyOut = Tables.resolve_var_name key' in
            let tvalOut =
              try
                Hashtbl.find foundvars (id,i)
            with Not_found ->
                Hashtbl.find foundvars (id,-(i+1)) in
            let keyvalOut =
              if Str.string_match reg3 tvalOut 0 then "1"
              else if Str.string_match reg4 tvalOut 0 then "0"
              else if Str.string_match reg5 tvalOut 0 then "0"
              else if Str.string_match reg6 tvalOut 0 then "1"
              else  tvalOut in
            Buffer.add_string buf2 ("        <Value name=\""^keyOut^"\" type=\"output\" step=\""^(string_of_int (i+maxstep))^"\">"^keyvalOut^"</Value>\n");
          )else(
             ...
             let tval =
               ...
               with Not_found ->
                 Hashtbl.find foundvars (id,-(i+1)) in
             let keyval =
              if Str.string_match reg3 tval 0 then "1"
              else if Str.string_match reg4 tval 0 then "0"
              else if Str.string_match reg5 tval 0 then "0"
              else if Str.string_match reg6 tval 0 then "1"
              else  tval in
            Buffer.add_string buf2 ("        <Value name=\""^key^"\" type=\"local\" step=\""^(string_of_int (i+maxstep))^"\">"^keyval^"</Value>\n");
          )
          with Not_found ->
               let keyIn = Tables.resolve_var_name key' in
               match c with
               | INPUT ->  Buffer.add_string buf2 ("        <Value name=\""^keyIn^"\" type=\"input\" step=\""^(string_of_int (i+maxstep))^"\"> _ </Value>\n");
               | OUTPUT -> Buffer.add_string buf2 ("        <Value name=\""^keyIn^"\" type=\"output\" step=\""^(string_of_int (i+maxstep))^"\"> _ </Value>\n");
               | LOCAL -> Buffer.add_string buf2 ("        <Value name=\""^keyIn^"\" type=\"local\" step=\""^(string_of_int (i+maxstep))^"\"> _ </Value>\n");
               | _ -> ()
        done;

        Buffer.add_string buf (Buffer.contents buf2)
    ) (Tables.get_used_vars());
      Buffer.add_string buf "    </testTrace>\n";
      Buffer.add_string buf "</Results>\n";
      Buffer.contents buf


let printTestInput foundvars minstep maxstep =
  let minstep',maxstep' =
  ...
  Hashtbl.iter (fun id _ ->
    let (_,key',_,c) =
      Tables.safe_find_varinfo id "printTestInput"
    in
    if c = INPUT then
      begin
        ...
        if !found then print_string (Buffer.contents buf)
      end
    ) (Tables.get_used_vars())
...
let print_countermodel foundvars minstep maxstep =
  let minstep',maxstep' =
    if !do_negative_index then
      ...
      minstep,maxstep
  in
  let prev_loud = !Flags.loud in
  if !Flags.final_cex_loud then
    Flags.loud := true;
...
let print_header_xml fn =
 ...
 let mk_cex_xml foundvars minstep maxstep name k time =
   ...
   let prev_loud = !Flags.loud in
    ...
    let _  = Buffer.add_string buf "    <Answer>CEX</Answer>\n" in
        ...
        | L_INT_RANGE (x, y)  -> let lower = string_of_int(x) in
            ...
            let keyval =
              if Str.string_match reg3 tval 0 then
                ...
                tval
	    in
	      if String.length keyval < 3 then Buffer.add_string buf3 " ";
	      if String.length keyval < 2 then Buffer.add_string buf3 " ";
	      found:=true;
...
let print_resultProp_xml  xml_format =
  match xml_format.result with
      VALID -> mk_xml_result "SAFE" xml_format.k xml_format.time xml_format.name false
    ...
    | TIMEOUT -> mk_xml_result "TIMEOUT" xml_format.k xml_format.time xml_format.name true
    | _ -> assert false
...
let print_stat_headers () =
  ...
  if !aggressive_inlining > 0 then
    print_to_user(solver#get#cc^"MODE:AGGRESSIVE_INLINING_"^(string_of_int !Flags.aggressive_inlining)^" ("^(string_of_int (Inlining.get_number_variables_inlined()))^" variables inlined)\n")
